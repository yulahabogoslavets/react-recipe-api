...
export function RecipeFilters({
    ...
    useEffect(() => {
        if (filterRef.current) {
            setMaxHeight(
                showFilters ? `${filterRef.current.scrollHeight}px` : '0px'
            )
        }
    ...
    return (
        <>
            ...
            <div
                id="filters-panel"
                ref={filterRef}
                className={`my-2 flex flex-col gap-4 overflow-hidden transition-all duration-500 ease-in-out md:!max-h-full md:flex-row md:!opacity-100`}
                style={{
